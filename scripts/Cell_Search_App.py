            ...
            children=[
                ...
                html.Div(
                    ...
                    children=[
                        ...
                        html.Div(id='neuroglancer-open-trigger', style={'display': 'none'}),
...
def highlight_cell(search_clicks, reset_clicks, cell_id, id_type):
    ...
    if triggered_id == 'search-button' and cell_id:
        ...
        logger.info(
            ...
            plot_data.loc[plot_data['nucleus_id'] == cell_id].predicted_subclass.values[0],
        )
...
def handle_neighbor_and_copy(neighbor_clicks, copy_clicks, cell_id, n_neighbors, id_type):
    ctx = dash.callback_context
    if not ctx.triggered:
        return default_scatter_fig, default_neighbor_fig, {'display': 'none'}, dash.no_update
    ...
    if triggered_id == 'neighbor-button' and neighbor_clicks and n_neighbors and n_neighbors.isdigit():
        ...
        # Copy the soma_neighbors to clipboard in browser via clientside callback
        seg_ids = utils.get_latest_seg_ids(nuc_neighbors)
        return dash.no_update, dash.no_update, dash.no_update, ', '.join(map(str, seg_ids))
