# cell_search Development Guide

This guide documents the development workflow, testing strategies, and coding standards for cell_search.
...
- **Testing**: `poe test` → `uv run pytest --cov=cell_search tests`
...
# Coverage report
uv run pytest --cov=cell_search --cov-report=html tests/
```
...
This automatically:
- Updates version in `pyproject.toml` and `src/cell_search/__init__.py`
